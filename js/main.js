const BASE_URL = `https://javascript-25g-1063e-default-rtdb.firebaseio.com/`;
...
const savePet = async (pet, petId) => {
    if (petId) {
        let response = await fetch(`${BASE_URL}/Mascotas/${petId}/.json`, {
            method: "PUT",
...
const modifiedPet = async (petKey) => {
    petId = petKey;
    let newResponse = await fetch(`${BASE_URL}/Mascotas/${petKey}/.json`);
